...
def new_code_templet(AIM_M, AIM_F, PUN_M, PUN_F, FieldD, problem, maxormin, MAXGEN, NIND, SUBPOP, GGAP, selectStyle,
                         recombinStyle, recopt, pm, distribute, proba, result, drawing=1):
    """
...
"""
    ...
    [ObjV, LegV] = aimfuc(variable, LegV, proba, result)  # 求初代种群的目标函数值
    ...
    while gen < MAXGEN:
        ...
        [ObjVSel, LegVSel] = aimfuc(variable, LegVSel, proba, result)  # 求初代种群的目标函数值
        # 父子合并
    ...
    weightlist =[]
    for i in range(NVAR):
        weightlist.append(var_trace[best_gen, i])
        print(var_trace[best_gen, i])
    weightarray = array(weightlist)
    print('有效进化代数：%s' % (pop_trace.shape[0]))
    print('最优的一代是第 %s 代' % (best_gen + 1))
    print('时间已过 %s 秒' % (times))
    # 返回进化记录器、变量记录器以及执行时间
    return [weightarray, pop_trace, var_trace, times]


def new_code_templetX(AIM_M, AIM_F, PUN_M, PUN_F, FieldD, problem, maxormin, MAXGEN, NIND, SUBPOP, GGAP, selectStyle,
                         recombinStyle, recopt, pm, distribute, feature_data, result_data, drawing=1):
    """
sga_new_code_templet.py - 改进的单目标编程模板(二进制/格雷编码)
...
"""
    ...
    if problem == 'R':
        ...
        if np.any(FieldD >= sys.maxsize):
            ...
            variable = ga.bs2int(Chrom, FieldD).astype('int64')
    [ObjV, LegV, X_test_enc , Y_test] = aimfuc(variable, LegV, feature_data, result_data)  # 求初代种群的目标函数值
    start_time = time.time()  # 开始计时
    gen = 0
    badCounter = 0  # 用于记录在“遗忘策略下”被忽略的代数
    # 开始进化！！
    ...
    while gen < MAXGEN:
        ...
        if problem == 'R':
            ...
            if np.any(FieldD >= sys.maxsize):
                ...
                variable = ga.bs2int(SelCh, FieldD).astype('int64')
        LegVSel = np.ones((SelCh.shape[0], 1))  # 初始化育种种群的可行性列向量
        [ObjVSel, LegVSel] = aimfuc(variable, LegVSel, feature_data, result_data)  # 求初代种群的目标函数值
        # 父子合并
        Chrom = np.vstack([Chrom, SelCh])
        ObjV = np.vstack([ObjV, ObjVSel])
        LegV = np.vstack([LegV, LegVSel])
    ...
    print('时间已过 %s 秒' % (times))
    # 返回进化记录器、变量记录器以及执行时间
    return [weightarray, pop_trace, var_trace, times, X_test_enc, Y_test]
